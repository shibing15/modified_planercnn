    ...
    for name, detector in detectors:
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            if 'inference' in options.dataset:
                img_stem = os.path.splitext(os.path.basename(image_list[sampleIndex]))[0]
            else:
                img_stem = str(sampleIndex)
            ...
            if 'inference' not in options.dataset:
                ...
                for c in range(len(detection_pair)):
                    ...
                    # np.save(options.test_dir + '/' + str(sampleIndex) + '_plane_parameters_' + str(c) + '.npy',
                    #         detection_pair[c]['detection'][:, 6:9].cpu())
                    # np.save(options.test_dir + '/' + str(sampleIndex) + '_plane_masks_' + str(c) + '.npy',
            ...
            if sampleIndex < 30 or options.debug or options.dataset != '':
                ...
                visualizeBatchPair(options, config, input_pair, detection_pair, 
                                   indexOffset=img_stem, 
                                   suffix='_' + name + options.modelType, 
                                   write_ply=options.testingIndex >= 0, 
                                   write_new_view=options.testingIndex >= 0 and 'occlusion' in options.suffix)
                pass
            if sampleIndex >= options.numTestingImages:
                break
            continue
...
if __name__ == '__main__':
    ...
    if args.dataset == '':
        ...
        pass
    args.test_dir = '/media/ai-i-sunyunda/data/data/mne_slam/plane_rcnn/' + args.keyname
