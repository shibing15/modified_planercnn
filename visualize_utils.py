...
def visualizeBatchDetection(options, config, input_dict, detection_dict,
                            indexOffset=0, prefix='', suffix='',
                            prediction_suffix='', write_ply=False, write_new_view=False):
    image_dict = {}

    # 当前样本的编号，用你的 sampleIndex 传进来的那个
    img_id = str(indexOffset)

    # 1) 每一类一个文件夹
    base_dir = options.test_dir
    seg_dir = os.path.join(base_dir, "segmentation_final")
    depth_ori_dir = os.path.join(base_dir, "depth_final_ori")
    depth_np_dir = os.path.join(base_dir, "depth_final_np")
    depth_dir = os.path.join(base_dir, "depth_final")
    plane_masks_dir = os.path.join(base_dir, "plane_masks")
    for d in [seg_dir, depth_ori_dir, depth_np_dir, plane_masks_dir, depth_dir]:
    ...
    # 3) segmentation_final
    if 'detection' in detection_dict and len(detection_dict['detection']) > 0:
        detections = detection_dict['detection'].detach().cpu().numpy()
        detection_masks = detection_dict['masks'].detach().cpu().numpy().transpose((1, 2, 0))
        # 这里用 gt depth 只是为了喂给 draw_instances，不保存
        if 'depth' in input_dict:
            depth_gt = input_dict['depth'].detach().cpu().numpy()[0]
        else:
            depth_gt = np.zeros((image.shape[0], image.shape[1]), dtype=np.float32)
        ...
        cv2.imwrite(os.path.join(seg_dir, f"{img_id}.png"), instance_image[80:560])
    else:
        empty_seg = np.zeros_like(image[80:560], dtype=np.uint8)
        cv2.imwrite(os.path.join(seg_dir, f"{img_id}.png"), empty_seg)

    # 4) depth_xx_final_ori.png
    # 有 refine 前的就存 ori，没有就存当前的 depth
    if 'depth_ori' in detection_dict:
        depth_ori = detection_dict['depth_ori'].squeeze().detach().cpu().numpy()
        depth_ori_img = drawDepthImage(depth_ori[80:560])
        cv2.imwrite(os.path.join(depth_ori_dir, f"{img_id}.png"), depth_ori_img)
    elif 'depth' in detection_dict:
        depth_pred = detection_dict['depth'][0].detach().cpu().numpy()
        depth_pred_img = drawDepthImage(depth_pred[80:560])
        cv2.imwrite(os.path.join(depth_dir, f"{img_id}.png"), depth_pred_img)

    # 5) depth_xx_final_np.png
    if 'depth_np' in detection_dict:
        depth_np = detection_dict['depth_np'].squeeze().detach().cpu().numpy()
        depth_np_img = drawDepthImage(depth_np[80:560])
        cv2.imwrite(os.path.join(depth_np_dir, f"{img_id}.png"), depth_np_img)

    # 6) plane_masks.npy
    if 'masks' in detection_dict:
        masks_np = detection_dict['masks'][:, 80:560].detach().cpu().numpy()
        np.save(os.path.join(plane_masks_dir, f"{img_id}.npy"), masks_np)

    # 返回一张图，外面 visualizeBatchPair 还会用到
    image_dict['detection'] = image[80:560]
    return image_dict
